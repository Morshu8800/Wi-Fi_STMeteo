...
void BMP280_SetConfig(uint8_t standby_time, uint8_t filter) {
	BMP280_Write8(BMP280_CONFIG,
			(((standby_time & 0x7) << 5) | ((filter & 0x7) << 2)) & 0xFC);
}
//===============================================================================================
void BMP280_Init(I2C_HandleTypeDef *i2c_handler, uint8_t temperature_resolution,
		uint8_t pressure_oversampling, uint8_t mode) {
	t_err = HAL_GetTick();
	i2c_h = i2c_handler;
...
float BMP280_ReadTemperature(void) {
	...
	if (_mode == BMP280_FORCEDMODE) {
		...
		if (mode == BMP280_FORCEDMODE) {
			...
			var1 = ((((adc_T >> 3) - ((int32_t) t1 << 1))) * ((int32_t) t2))
					>> 11;

			var2 = (((((adc_T >> 4) - ((int32_t) t1))
					* ((adc_T >> 4) - ((int32_t) t1))) >> 12) * ((int32_t) t3))
					>> 14;
