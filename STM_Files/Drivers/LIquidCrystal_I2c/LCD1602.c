...
bool LCD_Connect_test(void) {
	///Функция проверки наличия LCD1602 на шине i2c
	///Вернет true, если по указанному адресу i2c, LCD даст ответ. false при неудаче.
	if (HAL_I2C_IsDeviceReady(&LCD_I2C_TYPEDEF, LCD_I2C_ADDR, 5, 100) == HAL_OK)
		return true;
	else
		return false;
}
